...
function validateAdc(title, application_id, application_group_id) {
  const errors = [];
...
// List all ADCs for current year, with embedded allocations and aggregate month totals
router.get('/', async (req, res) => {
  const year = new Date().getFullYear();
  const { rows } = await db.query(`
    SELECT
      a.id, a.title, a.description, a.year,
      a.application_id, a.application_group_id,
    ...
    LEFT JOIN application_groups ag ON ag.id = a.application_group_id
    LEFT JOIN adc_allocations al  ON al.adc_id = a.id
    LEFT JOIN ftes f              ON f.id = al.fte_id
    LEFT JOIN tbh_slots t         ON t.id = al.tbh_id
    WHERE a.year = $1
    GROUP BY a.id, app.name, ag.name
    ORDER BY target_name ASC, a.title ASC
  `, [year]);
  res.json(rows);
});
...
router.post('/', async (req, res) => {
  ...
  const errors = validateAdc(title, application_id, application_group_id);
  if (errors.length) return res.status(422).json({ errors });
  const year = new Date().getFullYear();
  const { rows } = await db.query(
    `INSERT INTO adcs (title, description, year, application_id, application_group_id)
...
router.put('/:id', async (req, res) => {
  ...
  if (!rows.length) return res.status(404).json({ errors: ['ADC not found'] });
  res.json(rows[0]);
});

// Delete ADC
router.delete('/:id', async (req, res) => {
  const { rowCount } = await db.query('DELETE FROM adcs WHERE id = $1', [req.params.id]);
  if (!rowCount) return res.status(404).json({ errors: ['ADC not found'] });
  res.status(204).end();
});
...
router.post('/:id/allocations', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO adc_allocations
         (adc_id, fte_id, tbh_id, jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, dec)
...
router.delete('/:id/allocations/:allocId', async (req, res) => {
  const { rowCount } = await db.query(
    'DELETE FROM adc_allocations WHERE id = $1 AND adc_id = $2',
    [req.params.allocId, req.params.id]
  );
